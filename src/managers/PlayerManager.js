...
class PlayerManager extends BaseManager {
  ...
  async #fetchMinimalByUsername(username) {
    const request = await fetch(`https://api-core.wolvesville.com/players/search?username=${username}`, {
      method: 'GET',
  ...
  async fetchById(id) {
    const request = await fetch(`https://api-core.wolvesville.com/players/${id}`, {
      method: 'GET',
      headers: getAuthenticationHeaders(this.client.token)
    });
